...
void do_tunnel(void *ip, void *op)
{
    ...
    while (1)
    {
        ...
        DBG_LOG("tunnel [%d] -> [%d] with %d bytes", pinfo->read_fd, pinfo->write_fd, n);
    }
    free(buffer);
...
void do_proxy(void *ip, void *op)
{
    ...
    if (escape)
    {
        DBG_LOG("%s:%d escaping through -> %s:%d",
                inet_ntoa(pinfo->intended_addr.sin_addr), ntohs(pinfo->intended_addr.sin_port),
                LOCALHOST, TUNNEL_BIND_PORT);
    }

    pinfo->target_fd = connect_target(target_addr);
    if (pinfo->target_fd <= 0)
    {
        DBG_LOG("connect to target %s:%d error %d",
                inet_ntoa(target_addr->sin_addr), ntohs(target_addr->sin_port), pinfo->target_fd);
...
void server(void *ip, void *op)
{
    ...
    while (1)
    {
        ...
        getsockname(client_fd, (struct sockaddr *)&(pinfo->intended_addr), &dest_addr_len);
